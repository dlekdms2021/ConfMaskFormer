...
class Strats(TimeSeriesModel):
    def __init__(self, args: Namespace):
        ...
        # 🔴 self-supervised forecast head (각 비콘별 값 예측)
        self.forecast_head = nn.Linear(self.ts_demo_dim, self.V)
    ...
    def forward(self, values, times, varis, obs_mask, demo,
        ...
        forecast_values / forecast_mask: self-supervised pretrain에서만 사용
        """
        ...
        # 🔴 self-supervised pretrain 모드 (forecast)
        if getattr(self.args, "pretrain", 0) == 1:
            # forecast_values: (B, V), forecast_mask: (B, V)
            if (forecast_values is None) or (forecast_mask is None):
                # 방어 코드: 타깃이 없으면 0 loss
                return torch.tensor(0.0, device=device, requires_grad=True)

            pred = self.forecast_head(ts_demo_emb)  # (B, V)
            mask = forecast_mask.bool()             # (B, V)

            if mask.sum() == 0:
                return torch.tensor(0.0, device=device, requires_grad=True)

            loss = F.mse_loss(pred[mask], forecast_values[mask])
            return loss

        # 🔵 supervised multi-class classification 모드
