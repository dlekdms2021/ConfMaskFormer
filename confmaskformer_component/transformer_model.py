...
class TransformerClassifierImproved(nn.Module):
    ...
    def __init__(self, n_beacons: int, n_classes: int, cfg,
                 ...
                 use_confidence_gate: bool = True, use_auxiliary_loss: bool = True,
                 use_beacon_dropout: bool = True, use_combined_input: bool = True):
        """
        ...
        use_confidence_gate: Confidence Gate 활성화 여부
        use_auxiliary_loss: Auxiliary Reconstruction Loss 활성화 여부
        use_beacon_dropout: Beacon Dropout 활성화 여부
        use_combined_input: RSSI + Mask 결합 입력 활성화 여부
        """
        super().__init__()
        self.cfg = cfg                                 # 설정 저장
        ...
        self.use_beacon_dropout = use_beacon_dropout
        self.use_combined_input = use_combined_input

        # 결합 입력 여부에 따라 입력 차원 결정
        in_dim = n_beacons * 2 if use_combined_input else n_beacons  # RSSI(5) + 마스크(5) 또는 RSSI(5)만
        self.embedding = nn.Linear(in_dim, cfg.embedding_dim)  # (10→D) 또는 (5→D) 임베딩 투영
    ...
    def _maybe_beacon_dropout(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        if not self.use_beacon_dropout:  # Beacon Dropout이 비활성화되면 원본 유지
            return x
    ...
    def forward(self, x: torch.Tensor, do_aux: bool = True):
        ...
        # Confidence Gate (옵션): 시점별 신뢰도 게이트 계산
        conf = confidence_scale_from_mask(obs_mask) if self.use_confidence_gate else torch.ones(B, T, 1, device=x.device)

        # 입력 구성 (옵션): 마스크 결합 여부에 따라 결정
        if self.use_combined_input:
            x_in = torch.cat([x, obs_mask], dim=-1)              # (B,T,10=5값+5마스크)
        else:
            x_in = x                                             # (B,T,5) RSSI만 사용

        x_in = self.embedding(x_in)                          # (B,T,D)로 투영
        x_in = self.pos(x_in)                                # 위치 정보 더하기
        x_in = x_in * conf                                   # Confidence Gate 적용 (또는 1로 곱해짐)
        ...
        # 보조 복원 손실 (옵션): 학습 시에만, do_aux=True이고 비율>0일 때
        if self.training and do_aux and self.use_auxiliary_loss and self.aux_mask_ratio > 0:
            ...
            if valid.any():
                ...
                conf2 = confidence_scale_from_mask(obs2) if self.use_confidence_gate else torch.ones(B, T, 1, device=x.device)
